...
# ════════════════════════════════════════════════════════════
# 统一 Prompt 构建（选择题专用，单次API调用）
# ════════════════════════════════════════════════════════════

def build_choice_prompt(full_text: str, precise_cks: list, laws: list, triples: list) -> str:
    """
    为选择题构建统一 Prompt
    ...
    - full_text: 题目完整文本
    - precise_cks: 精确法条列表
    - laws: Lawformer 检索结果（最多1个）
    - triples: 知识图谱三元组（最多3个）
    """
    ...
    # 格式化检索法律部分（如果有精确命中就不显示）
    laws_section = ""
    if laws and not precise_cks:
        laws_section = "═══ 参考法律（Lawformer检索）═══\n"
        for law in laws:
            laws_section += f"【{law['law_name']}】相似度 {law['similarity']}\n"
            laws_section += f"相关内容：{law['matched_text'][:300]}\n\n"

    # 格式化知识图谱部分（如果有精确命中就不显示）
    kg_section = ""
    if triples and not precise_cks:
        kg_section = "═══ 知识图谱三元组（仅供参考）═══\n"
        for i, t in enumerate(triples, 1):
            kg_section += f"{i}. {t['head']} --[{t['relation']}]--> {t['tail']} (评分: {t['score']})\n"
        kg_section += "\n"
...
def process_item(item: dict, task_name: str) -> dict:
    ...
    # Step 1: Lawformer 检索 → 只取第1个最相似的
    laws: list = []
    try:
        all_laws = get_law_retriever().search(full_text)
        laws = all_laws[:1]  # ✅ 只取最像的一个
    except Exception as e:
        logging.warning(f"[{task_name}] Lawformer search failed: {e}")
    ...
    return {
        ...
        "kg_triples": [{"head": t["head"], "relation": t["relation"],
                        "tail": t["tail"], "score": t["score"]} for t in top_triples],
        "precise_chunks": precise_cks,
        "api_calls": 1,  # ✅ 明确标注：1次 API 调用
    }
...
def run_on_file(f_path: str):
    ...
    print(f"    total_api_calls: {total_api_calls} (平均 {total_api_calls / len(results):.1f}次/样本)")


def main():
    print("=" * 70)
    print("GNN-Chain Pipeline - 选择题专用版（优化）")
    print("=" * 70)
    print("✨ 核心优化：")
    print("  1. 单次 API 调用（替代原来的两次）")
    print("  2. Lawformer 只取最相似的 1 个法律")
    print("  3. 知识图谱只取评分最高的 3 个三元组")
    print("  4. 如果精确命中法条，自动清空 KG（减少噪声）")
    print("  5. API 重试次数降低为 3 次")
    print("=" * 70)
    ...
    if avail:
        ...
        print("  ⚠️  未找到 KG 文件，请检查 MODEL_DIR 和 KG_DIR 配置")
