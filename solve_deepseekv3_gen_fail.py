...
def fix_jsonl(out_path: str, data_path: str, task_name: str):
    """
    1. 读取输出 JSONL，收集 FAILED 条目及其索引
    2. 用 input 字段匹配原始数据，重建 prompt 重新请求
    3. 原地回写整个 JSONL 文件
    """
    # ── Step 1: 读取当前输出 JSONL ──────────────────────────
    records = []
    ...
    if not failed_indices:
        print(f"  [SKIP] {task_name}: no FAILED entries found.")
        return

    print(f"\n  [FIX]  {task_name}: {len(failed_indices)} FAILED item(s) found at index {failed_indices}")
    logging.info(f"[{task_name}] Fixing {len(failed_indices)} failed items: indices={failed_indices}")

    # ── Step 2: 建立 input -> 原始数据项 的映射 ─────────────
    raw_data = load_data(data_path)
    # 用 input 字段做 key（若有重复 input 则取第一条，极少见）
    input_to_raw = {}
    for item in raw_data:
        key = item.get('input', '')
        if key not in input_to_raw:
    ...
    for idx in failed_indices:
        ...
        print(f"    Re-querying index {idx} | input[:60]={item_input[:60]!r}")
    ...
    if still_failed == 0:
        print(f"  [DONE] {task_name}: all items fixed. File updated: {out_path}")
        logging.info(f"[{task_name}] All failed items fixed successfully.")
    else:
        print(f"  [WARN] {task_name}: {still_failed} item(s) still FAILED after fix.")
        logging.warning(f"[{task_name}] {still_failed} items still FAILED after fix.")
...
# ════════════════════════════════════════════════════════════
def main():
    out_files = sorted(glob.glob(os.path.join(OUT_DIR, '*.jsonl')))
    if not out_files:
        print(f"No output JSONL files found in {OUT_DIR}")
        return

    print(f"Scanning {len(out_files)} output files for FAILED entries...\n")

    fixed_count = 0
    for out_path in out_files:
        # 从文件名还原 task_name，例如 deepseek_v3_4_1.jsonl -> 4_1
        basename  = os.path.basename(out_path)              # deepseek_v3_4_1.jsonl
        task_name = basename.replace('deepseek_v3_', '').replace('.jsonl', '')  # 4_1
    ...
    print(f"\nAll done. Fixed {fixed_count} file(s).")
    logging.info(f"Fix script completed. Fixed {fixed_count} file(s).")
